define([
  ...
  function ValidateFields(Method, StepActual, data) {
    ...
    if (Method == 'Create') {
      ...
      if (StepActual == '5') {
        if (($('.updatepass input[type=text]').val() === 'Undefined' || $('.updatepass input[type=text]').val().length === 0) || ($('.pushpass textarea').val() === 'Undefined' || $('.pushpass textarea').val().length === 0) || ($('.pushpass select').find('option:selected').attr('value').trim() === 'Undefined' || $('.pushpass select').find('option:selected').attr('value').trim().length === 0)) {
          FlagFields(Method);
        } else {
          WriteSummary(Method);
          connection.trigger('nextStep');
